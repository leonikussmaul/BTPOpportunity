...
sap.ui.define([
    ...
    function (UriParameters, UIComponent, Device, models, JSONModel, library, FlexibleColumnLayoutSemanticHelper, Component) {
        "use strict";
        ...
        return UIComponent.extend("opportunity.opportunity.Component", {
            ...
            init: function () {
                ...
                // Tell UI5 where to find the components
                this.setResourceRoots();
            ...
            setResourceRoots: function () {

                // component name (with dots) to path (with /)

                var compName = "chatbot";
                var localUrl = './components/chatbot';
                // var componentUrl = 
                // '/ngsplibrary1/comp/planoverview'

                // Is it really there in our UI project
                var sLocalProjectPath = localUrl + '/Component.js';
                var isFound = false;
                $.get({
                    url: sLocalProjectPath,
                    async: false,
                    success: function (data) {
                        isFound = true;
                    },
                ...
                if (isFound === true) {
                    // set the resource root to locally in the app
                   ...
                   // function to load the components that are not lazy loaded
                   preLoadComponents: function () {

                    const aCreateComponentsPromises = [];
                
                     
                            //var isStopped = this.getRouter().isStopped();
                            var createPromise = Component.create({
                                name: 'chatbot',
                                id: "myId-" + 'chabot'
                               // settings: { isRunAsComponent: true }
    
                            });
    
                            aCreateComponentsPromises.push(createPromise);
    
                
                        Promise.all(aCreateComponentsPromises).then(values => {
    
                            values.forEach(comp => {
                                try {
                                  //  if (typeof comp.setIsRunAsComponent !== "undefined") {
                                        // comp.setIsRunAsComponent(true);
                                        var isRunAsComponent = comp.getIsRunAsComponent();
                                        var debug = "hello";
                                        comp.startRouter();
                                    //};
